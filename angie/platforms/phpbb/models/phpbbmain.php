...
class AngieModelPhpbbMain extends AngieModelBaseMain
{
	/**
	 * Try to detect the phpBB version in use
	 */
	public function detectVersion()
	{
		$ret = '0.0.0';

		$filename = APATH_ROOT . '/includes/constants.php';

		if (file_exists($filename))
		{
			// Let's pretend to be inside phpBB
			if(!defined('IN_PHPBB'))
			{
				define('IN_PHPBB', 1);
			}
	...
	public function getRequired()
	{
        ...
        if (empty($phpOptions))
        {
            ...
            if (extension_loaded( 'mbstring' ))
            {
                ...
                $option = array (
                    'label'		=> AText::_( 'MAIN_LBL_REQ_MBSTRINGINPUT' ),
                    'current'	=> (strtolower(ini_get('mbstring.http_input')) == 'pass'),
                    'warning'	=> false,
                );
                $option['notice'] = $option['current'] ? null : AText::_('MAIN_MSG_NOTICEMBSTRINGINPUT');
                $phpOptions[] = $option;

                $option = array (
                    'label'		=> AText::_( 'MAIN_LBL_REQ_MBSTRINGOUTPUT' ),
                    'current'	=> (strtolower(ini_get('mbstring.http_output')) == 'pass'),
                    'warning'	=> false,
                );
                $option['notice'] = $option['current'] ? null : AText::_('MAIN_MSG_NOTICEMBSTRINGOUTPUT');
                $phpOptions[] = $option;
            }
            ...
            $phpOptions[] = array (
                'label'		=> AText::_('MAIN_LBL_REQ_IMAGESIZE'),
                'current'	=> function_exists('getimagesize'),
                'warning'	=> false,
            );
	        ...
	        $phpOptions[] = array (
		        'label'		=> AText::_('MAIN_LBL_REQ_PCRE_UTF8'),
		        'current'	=> @preg_match('//u', ''),
		        'warning'	=> false,
	        );
