...
class AUtilsSerialised
{
	 ...
	 * @return string
	 */
	public static function replaceWithRegEx($serialized, $from, $to)
	{
		$pattern  = '/s:(\d{1,}):\"/iU';
		...
		$exploded = array_map(function ($piece) use (&$lastLen, $from, $to) {
			...
			$toReplace = str_replace($from, $to, $toReplace);
	...
	/**
	 * Checks whether a given array looks like a proper decoded serialised value
	 *
	 * @param   array  $arr  The array to check
	 *
	 * @return  bool  True if it looks like a proper decoded serialised value
	 */
